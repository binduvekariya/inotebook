...
const NoteState = (props) => {
  ...
  const editNote = async (id, title, description, tag) => {
    ...
    // iterate through all the notes and set element as the cuurentnote for every iteration i.e 1st iteration element = newNotes[0] ,2nd iteration element = newNotes[1],etc

    for (let index = 0; index < newNotes.length; index++) {
      const element = newNotes[index];
      //check if the _id of note[0] iterated matches with id passed  //if it matches then set newNotes[0].title = title(passed in function),etc
      if (element._id === id) {
